...
class ImageCropOverlay extends React.Component<Props, State> {
  ...
  calcRect() {
    ...
    const calculator = new CropRectCalculator(
      new Rect(initialTop, initialLeft, initialWidth, initialHeight),
  ...
  getTappedItem(x: number, y: number) {
    ...
    const yPercent =
      (y - initialTop - this.props.safeAreaHeight + this.props.scrollOffsetY) /
      initialHeight;
  ...
  handlePanResponderGrant = (
    ...
    const selectedItem = this.getTappedItem(
      e.nativeEvent.pageX,
      e.nativeEvent.pageY,
    );
    if (selectedItem == "tl") {

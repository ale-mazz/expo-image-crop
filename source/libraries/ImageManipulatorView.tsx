...
type Props = {
  borderColor?: string;
  isVisible: boolean;
  onPictureChosen: (data: ChosenPicture) => void;
  onBeforePictureChosen?: (data: ChosenPicture) => boolean;
  btnTexts: {
    crop?: string;
    rotate?: string;
    done?: string;
...
class ImageManipulatorView extends Component<Props, State> {
  ...
  constructor(
    props: Omit<Props, "borderColor" | "btnTexts"> &
      typeof ImageManipulatorView.defaultProps,
  ) {
    super(props);
  ...
  onRotateImage = async () => {
    ...
    Image.getSize(uri, async (width, _height) => {
      const { uri: rotUri, base64 } = await this.rotate(uriToCrop);
  ...
  rotate = async (uri: string): Promise<ImageManipulator.ImageResult> => {
    const { saveOptions } = this.props;
    return await ImageManipulator.manipulateAsync(
      uri,
      [{ rotate: -90 }],
  ...
  crop = async (
    cropObj: ImageManipulator.ActionCrop["crop"],
    uri: string,
  ): Promise<ImageManipulator.ImageResult> => {
    const { saveOptions } = this.props;
    if (cropObj.height > 0 && cropObj.width > 0) {
